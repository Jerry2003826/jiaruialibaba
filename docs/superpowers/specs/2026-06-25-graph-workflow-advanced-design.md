...
| Type | Behavior |
|------|----------|
| `subgraph` | Load saved definition by `definitionId` (+ optional `version`), run nested workflow, merge outputs |
...
- `WorkflowInlineExecutionService`: loop body trace, subgraph nested run, dynamic tools
