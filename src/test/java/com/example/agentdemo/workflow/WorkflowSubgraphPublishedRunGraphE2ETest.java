...
@SpringBootTest(classes = AgentBackendDemoApplication.class, properties = {
        ...
        "demo.workflow.require-published-for-run=true",
        "spring.ai.dashscope.api-key=",
        "spring.datasource.url=jdbc:h2:mem:subgraph_published_run_graph_e2e;MODE=PostgreSQL;DATABASE_TO_LOWER=TRUE;DB_CLOSE_DELAY=-1;DB_CLOSE_ON_EXIT=false",
        "spring.jpa.properties.hibernate.dialect=org.hibernate.dialect.H2Dialect",
        "demo.dashvector.endpoint=",
        "demo.dashvector.api-key="
})
class WorkflowSubgraphPublishedRunGraphE2ETest {
    ...
    @Test
    void runsPublishedSubgraphWorkflowWhenPublishRequired() {
        ...
        WorkflowDefinitionResponse child = workflowDefinitionService.save(new WorkflowDefinitionSaveRequest(
                "published-child-subgraph", "child", childDefinition));
        workflowDefinitionService.publish(child.definitionId());
        ...
        WorkflowDefinitionResponse parent = workflowDefinitionService.save(new WorkflowDefinitionSaveRequest(
                "published-parent-subgraph", "parent", parentDefinition));
        workflowDefinitionService.publish(parent.definitionId());

        WorkflowRunResponse run = workflowService.run(new WorkflowRunRequest(null, parent.definitionId(), Map.of()));

        assertThat(traceService.listSteps(run.runId()))
