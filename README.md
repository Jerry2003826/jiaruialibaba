...
- `GraphWorkflowRuntime`: 使用 Spring AI Alibaba `StateGraph` / `CompiledGraph` 执行线性节点和 `condition` true/false 条件分支；遇到 `parallel` / `join` 会返回 `WORKFLOW_UNSUPPORTED`
...
- `parallel`: 受限并行分支入口，至少两条普通 outgoing edge，每条分支会在 `simple` runtime 中并发执行。
...
- `graph` runtime 当前只支持普通边和 `condition` true/false 条件边；还不支持 `parallel` / `join`、循环、子图或动态节点展开。
...
- 当前 Graph 接入已经覆盖线性 DAG 和最小条件分支；`parallel` / `join` 目前只在 `simple` runtime 支持，Graph runtime 会显式拒绝。后续要继续补 Graph 并行、子图、循环防护策略和状态合并策略。
