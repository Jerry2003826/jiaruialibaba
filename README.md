...
该接口会读取 `workflow_run_records` 中的 definition/version 索引，加载当次运行使用的 workflow definition revision，再叠加 `/api/runs/{runId}/steps` 中的节点执行状态，返回 `summary`、`nodes`、`edges` 和 `mermaid`。`nodes` 会标记 `executed`、`status`、`stepId` 和 `errorMessage`；`edges` 会标记是否 `traversed`，未执行分支会用虚线 Mermaid edge 表示。它不会执行节点、不会调用模型/RAG/工具，也不会创建新的 run trace。

当前这个图导出只支持通过已保存 workflow definition 运行后写入 `workflow_run_records` 的 run。inline workflow run 仍然可以通过 `/api/runs/{runId}` 和 `/api/runs/{runId}/steps` 查看原始 trace，但暂时不会出现在 `/api/workflows/runs/{runId}/graph`。
...
- `parallel` 分支内部暂不支持嵌套 `condition`、嵌套 `parallel`、子图或动态节点展开。
- 非 `condition` / `parallel` 节点不能有多条 outgoing edge，复杂图会返回 `WORKFLOW_UNSUPPORTED`。
...
- `graph` runtime 当前还不支持循环、子图或动态节点展开。
- 当前不支持循环、子图、动态节点展开或任意表达式执行。
...
- `GET /api/workflows/runs/{runId}/graph` 依赖 `workflow_run_records` 定位 definition revision，因此当前只支持已保存定义触发的 run。
...
- `GET /api/workflows/runs/{runId}/graph` 可用于运行后图结构检查，返回节点执行状态、未执行分支和 Mermaid 文本。
